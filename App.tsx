...
import { Terminal as TerminalIcon, FileText, Image as ImageIcon, LayoutGrid, Power, Gamepad, Settings as SettingsIcon, RefreshCw, FolderPlus, Monitor, Music, LayoutTemplate, Rabbit, Calculator } from 'lucide-react';
...
const DEFAULT_WALLPAPER = 'https://images.unsplash.com/photo-1550684848-fac1c5b4e853?auto=format&fit=crop&q=80';
const LOGIN_WALLPAPER = 'https://images.unsplash.com/photo-1477346611705-65d1883cee1e?auto=format&fit=crop&q=80';
...
const App = () => {
  ...
  useEffect(() => {
    ...
    preloadImage(DEFAULT_WALLPAPER);
    preloadImage(LOGIN_WALLPAPER);
  }, []);
  ...
  // ---- Layered Rendering for Seamless Transitions ----

  return (
    <div className="h-full w-full bg-black overflow-hidden relative font-sans text-white">
        
        {/* Layer 1: DESKTOP (Always rendered if ON, just covered by lockscreen) */}
        {bootState !== BootState.OFF && (
            <div 
                className="absolute inset-0 z-0 bg-cover bg-center overflow-hidden transition-[background-image] duration-1000 ease-in-out"
            ...
            >
                {/* Desktop Icons */}
                <div className="absolute top-6 left-6 flex flex-col gap-8 z-10">
                ...
                <div className="absolute inset-0 z-20 pointer-events-none">
                     {/* Pass pointer-events-auto inside Window component */}
                    {windows.map(window => (
                        <Window 
                            key={window.id} 
                            window={window}
                ...
                {contextMenu.isOpen && (
                    <div 
                        className="absolute z-[9999] bg-slate-800/90 backdrop-blur-xl border border-white/10 p-1.5 rounded-lg shadow-2xl w-56 text-gray-200 text-xs animate-in fade-in zoom-in-95 duration-100 origin-top-left pointer-events-auto"
                ...
                {/* Start Menu */}
                <div className={`absolute bottom-14 left-4 w-[600px] h-[650px] bg-slate-900/90 backdrop-blur-2xl rounded-xl border border-white/10 shadow-[0_0_50px_rgba(0,0,0,0.5)] transition-all duration-300 ease-[cubic-bezier(0.2,0,0,1)] flex flex-col z-[9999] origin-bottom-left ${startMenuOpen ? 'scale-100 opacity-100 translate-y-0' : 'scale-95 opacity-0 translate-y-8 pointer-events-none'}`}
                    onClick={(e) => e.stopPropagation()}
                >
        ...
        {/* Layer 2: LOGIN SCREEN (Slides up) */}
        {bootState !== BootState.OFF && (
            <div 
                className={`absolute inset-0 z-[12000] bg-[url('https://images.unsplash.com/photo-1477346611705-65d1883cee1e?auto=format&fit=crop&q=80')] bg-cover bg-center flex items-center justify-center transition-transform duration-700 ease-[cubic-bezier(0.76,0,0.24,1)] ${bootState === BootState.DESKTOP ? '-translate-y-full pointer-events-none' : 'translate-y-0'}`}
            >
                <div className="absolute inset-0 bg-black/40 backdrop-blur-sm" />
                <div className={`relative z-10 flex flex-col items-center transition-opacity duration-500 ${bootState === BootState.DESKTOP ? 'opacity-0' : 'opacity-100'}`}>
                ...
                <div className={`absolute bottom-12 right-12 text-white text-right transition-all duration-1000 ${bootState === BootState.DESKTOP ? 'opacity-0 translate-y-10' : 'opacity-100'}`}>
        ...
        {/* Layer 3: BOOT SCREEN (Fades out) */}
        {bootState !== BootState.OFF && bootState !== BootState.DESKTOP && (
             <div 
                className={`absolute inset-0 z-[13000] bg-black flex flex-col items-center justify-center text-white cursor-none transition-opacity duration-1000 ${bootState === BootState.LOGIN ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}
             >
        ...
        {/* Layer 4: POWER OFF STATE */}
        {bootState === BootState.OFF && (
            <div className="absolute inset-0 z-[14000] bg-black flex items-center justify-center">
                <button 
